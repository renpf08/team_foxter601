...
typedef enum {
	BLE_REPORT_INIT,
	BLE_REPORT_ADV_START,
	BLE_REPORT_ADV_STOP,
	BLE_REPORT_CONNECT,
	BLE_REPORT_DISCONNECT,
    BLE_REPORT_PAIRING,
    BLE_REPORT_MATCHING,
	BLE_REPORT_MAX,
}BLE_REPORT_E;
...
s16 state_ble_adv_stop(REPORT_E cb, void *args)
{
    //print((u8*)&"adv stop", 8);
    pair_code.pair_bgn = 0;
    motor_notify_to_position(NOTIFY_NONE);
...
s16 state_ble_adv_start(REPORT_E cb, void *args)
{
...
static state_t ble_state_mc[] = {
	BLE_STATE_FILL(BLE_STATE_ADV_START, BLE_REPORT_ADV_STOP,    BLE_STATE_ADV_STOP,     state_ble_adv_stop), // back to clock state
	BLE_STATE_FILL(BLE_STATE_ADV_START, BLE_REPORT_CONNECT,     BLE_STATE_CONNECT,      state_ble_connect),
	BLE_STATE_FILL(BLE_STATE_ADV_STOP,  BLE_REPORT_ADV_START,   BLE_STATE_ADV_START,    state_ble_adv_start),
	BLE_STATE_FILL(BLE_STATE_CONNECT,   BLE_REPORT_ADV_START,   BLE_STATE_ADV_START,    state_ble_adv_start), // app disconnect
	BLE_STATE_FILL(BLE_STATE_CONNECT,   BLE_REPORT_ADV_STOP,    BLE_STATE_ADV_STOP,     state_ble_adv_stop), // m button disconnect
	BLE_STATE_FILL(BLE_STATE_CONNECT,   BLE_REPORT_PAIRING,     BLE_STATE_PAIRING,      state_ble_pairing),
	BLE_STATE_FILL(BLE_STATE_PAIRING,   BLE_REPORT_PAIRING,     BLE_STATE_PAIRING,      state_ble_pairing),
	BLE_STATE_FILL(BLE_STATE_PAIRING,   BLE_ADVERTISE,          BLE_STATE_ADV_START,    state_ble_adv_start), //app disconnect
	BLE_STATE_FILL(BLE_STATE_PAIRING,   BLE_REPORT_ADV_STOP,    BLE_STATE_ADV_STOP,     state_ble_adv_stop), // m button disconnect
};
static s16 state_ble_poll(REPORT_E cb, void *args)
{
	u16 i = 0;
...
s16 state_ble_change(REPORT_E cb, void *args)
{
    app_state state = ble_state_get();
    if((state == app_fast_advertising) || (state == app_slow_advertising)) {
        state_ble_poll(BLE_REPORT_ADV_START, args);
    } else if(state == app_idle){
        state_ble_poll(BLE_REPORT_ADV_STOP, args);
    } else if(state == app_connected){
        state_ble_poll(BLE_REPORT_CONNECT, args);
    } else {
        state_ble_poll(BLE_REPORT_DISCONNECT, args);
    }
